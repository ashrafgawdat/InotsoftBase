...
namespace Inorsoft.Base.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : PosServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("Inorsoft.Base", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
